  ...
  try
  {
    ...
    if ( bLastProject && !bInRepeatCalc && HasProjectRebateSkipFlags(pThis->m_Recordset) )
    {
      ...
      try
      {
        ...
        // Каждый нестабильный проход должен исключить
        // хотя бы одну новую позицию.
        ...
        for ( long nPass = 1; nPass <= nMaxRepeatCount; nPass++ )
        {
          ...
          */
          ...
          sRepeatProtocol.Format(_T("\r\n[PLUGIN] Проход распределения скидки %ld: " "исключено до = %ld, после = %ld."),
            nPass,
            nSkipBefore,
            nSkipAfter
          );
          ...
          /*
            Если новых исключений нет, этот проход уже
        ...
        {
          PluginAddProtocol(_T("\r\n[PLUGIN] Распределение скидки " "не стабилизировалось."));
        }
      }
      catch ( ... )
      {
