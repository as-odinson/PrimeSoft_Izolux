  ...
  try
  {
    ...
    if ( !sKomission.IsEmpty() )
    {
      ...
      for ( int i = 0; i < arrDocs.GetCount(); i++ )
      {
        ...
        nodePrd.SetAttribute(_T("НомерПРД"), arrDocs[i].sNum);
        nodePrd.SetAttribute(_T("ДатаПРД"), arrDocs[i].sDate);
      }
    }
    ...
    if ( !sANumCalcFact.IsEmpty() )
    {
      ...
      for ( int i = 0; i < arrDocs.GetCount(); i++ )
      {
        CXmlNode nodePrd = nodeDopSv.NewChild(_T("СопрДокФХЖ"));

        nodePrd.SetAttribute(_T("РеквНаимДок"), _T("АСЧФ"));
        nodePrd.SetAttribute(_T("РеквНомерДок"), arrDocs[i].sNum);
        nodePrd.SetAttribute(_T("РеквДатаДок"),  arrDocs[i].sDate);
      }
